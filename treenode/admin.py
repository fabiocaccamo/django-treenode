...
class TreeNodeModelAdmin(admin.ModelAdmin):
    ...
    def _get_treenode_field_display_with_accordion(self, obj):
        ...
        return format_html(
            ...
            tn_namespace_key, obj.pk, obj.tn_depth, obj.tn_level, obj_parent_id, obj_display
        )

    def _get_treenode_field_display_with_breadcrumbs(self, obj):
        items = [(obj_ancestor.get_display(indent=False),) for obj_ancestor in obj.get_ancestors()]
        parts = format_html_join(
            '',
            '<span class="treenode-breadcrumbs">{}</span>',
            items
        )
        return format_html('<span class="treenode">{}{}</span>', parts, obj.get_display(indent=False))

    def _get_treenode_field_display_with_indentation(self, obj):
        indentation = mark_safe('<span class="treenode-indentation">&mdash;</span>' * obj.ancestors_count)
        return format_html('<span class="treenode">{}{}</span>', indentation, obj.get_display(indent=False))
